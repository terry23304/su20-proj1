...
void readDictionary(char *dictName) {
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    char *word = malloc(sizeof(char) * read);
    strncpy(word, line, (read - 1));
...
void processInput() {
  ...
  while (c != EOF) {
    if (isalpha(c))
      ...
      // 原本的詞找一次，除了第一個轉小寫再找一次，全部轉小寫再找一次
      if (!findData(dictionary, word)) {
        char *lowercase = malloc(sizeof(char) * i);
        strncpy(lowercase, word, i);
        lowercase[i] = '\0';

        int j = 1;
        while (lowercase[j] != '\0') {
          lowercase[j] = tolower(lowercase[j]);
          j++;
        }

        if (!findData(dictionary, lowercase)) {
